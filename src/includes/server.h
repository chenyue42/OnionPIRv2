...
public:
  ...
  */
   ...
   */
  RlweCt evaluate_other_dim(std::vector<RlweCt> &intermediate_db, std::vector<GSWCiphertext> &selectors);

  // compute x = b * (y - x) + x
  void ext_prod_mux(RlweCt &x, RlweCt &y, GSWCiphertext &selection_cipher, RlweCt &result);
...
private:
  ...
  std::vector<RlweCt> expand_query(size_t client_id, seal::Ciphertext &ciphertext) const;
  ...
  std::vector<RlweCt> fast_expand_qry(size_t client_id, seal::Ciphertext &ciphertext) const;
