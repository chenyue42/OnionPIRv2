...
void mat_mat_32(const uint32_t *__restrict A, const uint32_t *__restrict B,
  uint64_t *__restrict out, const size_t rows, const size_t cols,
  const uint64_t mod) {
  // Max accumulations before the u64 accumulator could overflow.
  const uint64_t prod_max = (mod - 1) * (mod - 1);
  const size_t chunk = static_cast<size_t>(UINT64_MAX / prod_max);
  ...
  for (size_t i = 0; i < rows; i++) {
    ...
    while (k + chunk <= cols) {
      ...
      for (size_t j = 0; j < chunk; j++) {
        t0 += (uint64_t)A[offset + k + j] * B[2 * (k + j)];
        t1 += (uint64_t)A[offset + k + j] * B[2 * (k + j) + 1];
      }
      t0 %= mod;
      t1 %= mod;
      k += chunk;
    ...
    #pragma GCC unroll 32
    for (; k < cols; k++) {
      t0 += (uint64_t)A[offset + k] * B[2 * k];
      t1 += (uint64_t)A[offset + k] * B[2 * k + 1];
    }
    out[2 * i] = t0 % mod;
    out[2 * i + 1] = t1 % mod;
  }
...
void level_mat_mat_32(db_matrix32_t *A, db_matrix32_t *B, inter_matrix64_t *out,
  ...
  for (size_t level = 0; level < levels; ++level) {
    ...
    uint64_t *C_ptr = out_data + level * (m * 2);
    mat_mat_32(A_ptr, B_ptr, C_ptr, m, n, mod);
  }
}